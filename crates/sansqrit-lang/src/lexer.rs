...
pub enum TokenKind {
    ...
    // Keywords
    Let, Const, Fn, Return, If, Else, For, While, Loop,
    In, Break, Continue, Match, Class, Extends, Import, As,
    Simulate, Quantum, Classical, Circuit, Molecule, Struct,
    Try, Catch, Finally, Raise, And, Or, Not, Yield,
    ...
    FatArrow,   // =>
    Pipeline,   // |>
    Dot, DotDot, ColonColon,

    // Delimiters
    LParen, RParen, LBracket, RBracket, LBrace, RBrace,
    Comma, Colon, Semicolon, At, Hash,
...
impl Lexer {
    ...
    fn span(&self) -> Span {
        Span { line: self.line, col: self.col, offset: self.pos }
    }
    ...
    fn next_token(&mut self) -> Result<Token, LexError> {
        ...
        let kind = match ch {
            '+' => {
                if self.peek() == Some('=') { self.advance(); TokenKind::PlusAssign }
                else { TokenKind::Plus }
            }
            '-' => {
            ...
            '*' => {
                if self.peek() == Some('*') { self.advance(); TokenKind::DoubleStar }
                else if self.peek() == Some('=') { self.advance(); TokenKind::StarAssign }
                else { TokenKind::Star }
            }
            '/' => {
                if self.peek() == Some('/') { self.advance(); TokenKind::DoubleSlash }
            ...
            '!' => {
                ...
                else { return Err(LexError { msg: "Unexpected '!'".into(), span }); }
            }
            '<' => {
                if self.peek() == Some('=') { self.advance(); TokenKind::LtEq }
                else if self.peek() == Some('<') { self.advance(); TokenKind::ShiftLeft }
                else { TokenKind::Lt }
            }
            '>' => {
                if self.peek() == Some('=') { self.advance(); TokenKind::GtEq }
                else if self.peek() == Some('>') { self.advance(); TokenKind::ShiftRight }
                else { TokenKind::Gt }
            }
            '&' => TokenKind::Ampersand,
            '|' => {
                if self.peek() == Some('>') { self.advance(); TokenKind::Pipeline }
                else { TokenKind::Pipe }
            }
            '^' => TokenKind::Caret,
            '(' => TokenKind::LParen,
            ...
            '@' => TokenKind::At,
            _ => return Err(LexError { msg: format!("Unexpected character: '{}'", ch), span }),
        };
            ...
            match self.advance() {
                ...
                Some('\\') => {
                    match self.advance() {
                        ...
                        None => return Err(LexError { msg: "Unterminated escape".into(), span }),
                    }
                }
                Some(c) => s.push(c),
                None => return Err(LexError { msg: "Unterminated string".into(), span }),
            }
        }

        let kind = if is_fstring { TokenKind::FStringLit(s) } else { TokenKind::StringLit(s) };
        Ok(Token { kind, span })
    }
    ...
    fn lex_number(&mut self, span: Span) -> Result<Token, LexError> {
        ...
        if is_float {
            let val: f64 = s.parse().map_err(|_| LexError { msg: format!("Invalid float: {}", s), span })?;
            Ok(Token { kind: TokenKind::FloatLit(val), span })
        } else {
            let val: i64 = s.parse().map_err(|_| LexError { msg: format!("Invalid integer: {}", s), span })?;
