...
impl Parser {
    ...
    fn parse_shift(&mut self) -> Result<Expr, ParseError> {
        ...
        loop {
            let (op, matched) = match self.peek() {
                TokenKind::ShiftLeft => (BinOp::ShiftLeft, true),
                TokenKind::ShiftRight => (BinOp::ShiftRight, true),
                _ => (BinOp::ShiftLeft, false),
            };
            if !matched {
                break;
            }
            ...
            match self.peek() {
                ...
                TokenKind::ColonColon => {
                    ...
                    if let Expr::Ident(base, _) = &expr {
                        expr = Expr::ScopeResolution {
                            path: vec![base.clone(), method],
                            span,
                        };
                    }
                }
...
mod tests {
    ...
    fn test_bitwise_shift_expression() {
        let prog = parse("let flag = ((target >> i) & 1) == 0").unwrap();
        assert_eq!(prog.statements.len(), 1);
    }
}
